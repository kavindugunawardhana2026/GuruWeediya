...
export default function InterviewTracker({ interviews }: Props) {
    ...
    return (
      <Card className="flex flex-col items-center justify-center py-12 text-center animate-fade-in">
        ...
        <p className="text-sm text-slate-400 max-w-sm">
          When teachers apply to your jobs, you can schedule interviews with them here.
        </p>
      </Card>
    );
  }
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-white">
                    {interview.teacher?.full_name || "Unknown Teacher"}
                  </h3>
                  <p className="text-xs text-slate-400">
                    Applied for: <span className="font-medium text-slate-300">{interview.job?.title}</span>
                  </p>
                </div>
              ...
              {interview.status === "pending" && (
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => handleStatusChange(interview.id, "declined")}
                    disabled={isPending}
                    className="!px-3"
                    title="Decline"
                  >
